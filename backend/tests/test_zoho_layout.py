...
def test_plan_route_rejects_unsupported_section_creation(zoho, client, admin_token):
    resp = client.post(
    ...
    assert "cannot add a section" in resp.get_json()["error"]
    assert zoho["patches"] == []


def test_create_section_route_rejects_unsupported_operation(zoho, client, admin_token):
    resp = client.post(
        "/api/zoho/sections",
