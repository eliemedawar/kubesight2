...
/** One-shot sessionStorage hint: which Alerts tab to open on next mount (deep links). */
export const ALERTS_TAB_HINT_KEY = "kubesight.alerts.tab";

export function setAlertsTabHint(tab) {
  try {
    window.sessionStorage.setItem(ALERTS_TAB_HINT_KEY, tab);
  } catch {
    /* storage unavailable (private mode) — deep link degrades to the Open tab */
  }
}

export function consumeAlertsTabHint() {
  try {
    const hint = window.sessionStorage.getItem(ALERTS_TAB_HINT_KEY);
    if (hint) {
      window.sessionStorage.removeItem(ALERTS_TAB_HINT_KEY);
    }
    return hint || "";
  } catch {
    return "";
